"""SOLUTION — 02 — Write a context provider.  (module 4 · pairs with examples/04_memory.py)  (worked answer)
...
class TurnCounterProvider(ContextProvider):
    ...
    async def before_run(
        ...
        this_turn = state.get("turns", 0) + 1
        sentence = f"This is turn {this_turn} of the conversation."
        context.extend_instructions(self.source_id, sentence)
        # Stashed only so the exercise's self-check can see what was injected.
        state["last_injected"] = sentence
