"""SOLUTION — 07 — A guardrail that costs nothing.  (module 7 · pairs with examples/16_class_based_middleware.py)  (worked answer)
...
class RefusalMiddleware(AgentMiddleware):
    ...
    async def process(
        ...
        text = (last.text or "").lower() if last else ""
        if any(word in text for word in BANNED):
            # Setting result and NOT awaiting call_next() means the model is
            # never invoked: no tokens, no latency, nothing leaked.
