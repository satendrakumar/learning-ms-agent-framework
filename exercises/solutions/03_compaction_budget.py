"""SOLUTION — 03 — Compact well, not just hard.  (module 4 · pairs with examples/08_compaction_strategies.py)  (worked answer)
...
def build_strategy() -> TokenBudgetComposedStrategy:
    return TokenBudgetComposedStrategy(
        token_budget=BUDGET,
        tokenizer=TOKENIZER,
        # Excluding the old tool groups outright frees far more room than
        # summarising them, and unlike a sliding window it costs no real turns.
        # No blunt window on top: the tool-aware pass already fits the budget.
        strategies=[SelectiveToolCallCompactionStrategy(keep_last_tool_call_groups=0)],
    )
