"""Exercise 06 — Fan out, then merge.  (module 6 · pairs with examples/13_edge_patterns.py)
...
# TODO 3: fix this signature. What does a fan-in target actually receive?
@executor(id="report_card")
async def report_card(verdict: str, ctx: WorkflowContext[Never, str]) -> None:
    await ctx.yield_output(" | ".join(sorted(verdict)))


def build():
    builder = WorkflowBuilder(start_executor=intake)
    # TODO 1: intake -> length_check, todo_check, error_check  (in parallel)
    # TODO 2: those three -> report_card
